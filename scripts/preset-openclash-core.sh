...
# fetch its meta core into the cloned package so it ships inside the built
# package. The hook writes the package source's root/, upstream of packaging,
# so whichever format the tree emits (apk on current snapshots, ipk before)
